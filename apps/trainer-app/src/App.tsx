        ...
        <div className="card card-pad">
          ...
          <Field label="Recipient wallet">
            <div style={{ display: "flex", gap: 8, padding: "10px 12px", border: `1px solid ${T.border}`, borderRadius: 6, background: "#fff" }}>
              <span className="mono" style={{ flex: 1, color: T.ink, fontSize: 13 }}>0x6c41B27D3aE9…b18F</span>
          ...
          <Field label="Credential type">
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              {CREDENTIAL_TYPES.map((c, i) => (
                <div key={i} onClick={() => setSelectedCred(i)} style={{ padding: "12px 14px", borderRadius: 6, fontSize: 14, border: `2px solid ${i === selectedCred ? T.blueberry : T.divider}`, background: i === selectedCred ? "rgba(21,101,173,.05)" : "#fff", color: T.ink, fontWeight: i === selectedCred ? 600 : 500, cursor: "pointer" }}>
                  {c}
                </div>
              ))}
            </div>
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
            ...
            <button className="btn btn-primary btn-lg" style={{ flex: 1.4 }}>Issue credential ↗</button>
          </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div className="card" style={{ background: `linear-gradient(160deg, ${T.fig} 0%, #6e1547 100%)`, color: "#fff", padding: "24px 26px", borderColor: "transparent", position: "relative", overflow: "hidden" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "auto 1fr", gap: "4px 14px", marginTop: 16, fontSize: 13 }}>
              <span style={{ opacity: .75 }}>Holder</span>   <span className="mono">0x6c41…b18F</span>
              <span style={{ opacity: .75 }}>Issuer</span>   <span>WCK · 0x2A0e…1f44</span>
              <span style={{ opacity: .75 }}>Schema</span>   <span className="mono">WCK-Skill</span>
...
{`{
  credentialType:  "WCK-SafeFoodDistribution-L1",
  competencyArea:  "frontline-hot-meals",
  issuedAt:        1745020800,
  expiresAt:       1808092800,
  issuerOrg:       "World Central Kitchen",
  vcPayloadCid:    "bafyrei…ob4qm"
}`}
            </pre>
          </div>
